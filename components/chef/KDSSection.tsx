      ...
      <div className="flex gap-2 mb-5 -mx-4 px-4 overflow-x-auto no-scrollbar">
        {sections.map((s) => (
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all duration-200 flex-shrink-0 cursor-pointer ${
              activeTab === s.key
          ...
          {activeSection.items.map((order) => {
            ...
            return (
              <button
                ...
                className={`group relative bg-stone-900/80 border border-stone-800/80 rounded-xl p-3 md:p-4 text-left active:scale-[0.98] hover:border-amber-500/30 hover:shadow-lg hover:shadow-amber-500/5 transition-all duration-200 text-white overflow-hidden cursor-pointer ${statusColor} border-l-2`}
              >
        ...
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center">
          ...
          <div className="relative bg-stone-900 border border-stone-800 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md max-h-[85vh] flex flex-col overflow-hidden">
            ...
            <div className="flex items-center justify-between p-4 border-b border-stone-800 flex-shrink-0">
              ...
              <button onClick={() => { setSelectedOrder(null); setSelectedItem(null); }} className="p-1 text-stone-400 hover:text-white cursor-pointer">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="border-t border-stone-800 p-4 pb-[calc(1rem+env(safe-area-inset-bottom,0px))] flex-shrink-0">
              ...
              {(selectedOrder.status === "pending" || selectedOrder.status === "confirmed") && (
                <button onClick={() => handleStatus(selectedOrder.id, "preparing")} className="w-full py-3 bg-amber-500 text-black rounded-xl font-bold text-sm flex items-center justify-center gap-2 cursor-pointer hover:bg-amber-400 transition-colors">
                  <CookingPot size={16} />
                  Empezar preparación
                </button>
              )}
              {selectedOrder.status === "preparing" && (
                <button onClick={() => handleStatus(selectedOrder.id, "served")} className="w-full py-3 bg-green-600 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2 cursor-pointer hover:bg-green-500 transition-colors">
                  <CheckCircle size={16} />
                  Marcar listo
                </button>
              )}
              {selectedOrder.status === "served" && (
                <p className="text-center text-green-400 text-sm flex items-center justify-center gap-1 font-semibold">
